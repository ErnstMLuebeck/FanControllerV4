...
void StmacFanCtlr()
{
  if(FC_StFanCtlr == AUTO)
  {
      ...
      switch(FC_StFan)
      {
          case FANON: 
              // ABSCHALTEN WENN:
              // Taupunkt innen <= Taupunkt aussen + 2
              // ODER Temp. aussen <= TOutMin
              // ODER Luefter ist TiOnOff gelaufen
              // ODER Taupunkt Temp. aussen > Temp. innen - 5
              // ODER Sensor Error
              if(SI_TDewIn <= (SI_TDewOut+FC_TDewDiffOff) || SI_TOut <= FC_TOutMin || (now()-FC_TiFanStart) >= FC_TiFanOn || SI_TDewOut > (SI_TIn-5))
              {
          ...
          case FANOFF: 
              // EINSCHALTEN WENN:
              // Taupunkt aussen < (Taupunkt Innen - TTauDiffOn)
              // UND Taupunkt aussen < Temp. innen - 5
              // UND Temp. aussen > TOutMin
              // UND Luefter war TiOnOff aus     
              // UND kein Sensor Error   
              if(SI_TDewOut < (SI_TDewIn -FC_TDewDiffOn) && SI_TDewOut < (SI_TIn - 5)  && SI_TOut > FC_TOutMin && (now()-FC_TiFanStop) >= FC_TiFanOff)
              {
